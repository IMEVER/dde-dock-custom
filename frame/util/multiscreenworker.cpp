...
class SharedData {
    public:
        ...
        QRect getRect(const QString name, Position pos, qreal ratio, int itemSize, int itemCount)
        {
            return data.value(generalKey(name, pos, ratio, itemSize, itemCount), {});
        }

        void setRect(const QString name, Position pos, qreal ratio, int itemSize, int itemCount, QRect rect)
        {
            if(rect.isValid())
                data.insert(generalKey(name, pos, ratio, itemSize, itemCount), rect);
        }

        QString generalKey(const QString name, Position pos, qreal ratio, int itemSize, int itemCount)
        {
            return QString("%1_%2_%3_%4_%5").arg(name).arg(pos).arg(ratio).arg(itemSize).arg(itemCount);
        }
...
{
    m_animationTime = composite ? 300 : 0;
}
...
QRect MultiScreenWorker::dockRect(const QString &screenName, const Position &pos, const HideMode &hideMode)
{
    if (hideMode == HideMode::KeepShowing)
        ...
        return getDockHideGeometry(getDockShowGeometry(screenName, pos), pos);
}
...
QRect MultiScreenWorker::dockRectWithoutScale(const QString &screenName, const Position &pos, const HideMode &hideMode)
{
    if (hideMode == HideMode::KeepShowing)
        ...
        return getDockHideGeometry(getDockShowGeometry(screenName, pos, true), pos);
}
    ...
    if (interfaceName == "com.deepin.dde.daemon.Dock") {
        ...
        for (const QString &prop : keys) {
            if (prop == "Position") {
                ...
                if (m_position != position && position != Dock::Top)
                {
...
void MultiScreenWorker::showAniFinished()
{
    ...
    if(!m_parent->isHidden()) m_parent->setGeometry(rect);
...
void MultiScreenWorker::initConnection()
{
    ...
    connect(m_delayWakeTimer, &QTimer::timeout, this, [this]{
        // 移动Dock至相应屏相应位置
        if (testState(LauncherDisplay)) return;
...
void MultiScreenWorker::displayAnimation(const QString &screen, const Position &pos, AniAction act)
{
    ...
    ani->setDuration(m_animationTime);
    ...
    case AniAction::Hide:
        ...
        connect(ani, &QVariantAnimation::finished, m_parent, &MainWindow::hide);
        break;
    }
    ...
    connect(ani, &QVariantAnimation::stateChanged, this, [ this, act ](QAbstractAnimation::State newState, QAbstractAnimation::State oldState) {
        // 更新动画是否正在进行的信号值
        switch (act) {
        case AniAction::Show:
            if (newState == QVariantAnimation::Running && oldState == QVariantAnimation::Stopped) {
                m_parent->show();
                if (m_hideMode == HideMode::KeepShowing || m_animationTime)
                ...
                if (m_hideMode == HideMode::KeepShowing || m_animationTime)
                    ...
                    QTimer::singleShot(300, this, [ this ] { setStates(DockIsShowing, false); });
            }
            break;
        case AniAction::Hide:
            if (newState == QVariantAnimation::Running && oldState == QVariantAnimation::Stopped)
...
void MultiScreenWorker::changeDockPosition(QString fromScreen, QString toScreen, const Position &fromPos, const Position &toPos)
{
    ...
    ani1->setDuration(m_animationTime);
    ani2->setDuration(m_animationTime);

    //　隐藏
    QRect showRect = getDockShowGeometry(fromScreen, fromPos);
    ani1->setStartValue(showRect);
    ani1->setEndValue(getDockHideGeometry(showRect, fromPos));

    //　显示
    showRect = getDockShowGeometry(toScreen, toPos);
    ani2->setStartValue(getDockHideGeometry(showRect, toPos));
    ani2->setEndValue(showRect);
    ...
    if (fromPos != toPos)
        connect(ani1, &QVariantAnimation::finished, this, &MultiScreenWorker::requestUpdateLayout);
...
QRect MultiScreenWorker::getDockShowGeometry(const QString &screenName, const Position &pos, bool withoutScale)
{
    ...
    QRect rect = SharedData::instance()->getRect(screenName, pos, ratio, m_windowSize, dockCount);
    if(rect.isEmpty())
    {
        for (auto s : DIS_INS->screens()) {
        ...
        SharedData::instance()->setRect(screenName, pos, ratio, m_windowSize, dockCount, rect);
    }
...
void MultiScreenWorker::updateDockRect(QRect &dockRect, QRect screenRect, Position position, qreal ratio, int dockSize, int count)
{
    ...
    case Bottom:
            ...
            if ((dockSize + MODE_PADDING ) * count + splitterWidth > length * 0.9)
            {
                ...
                m_windowSize = round((length * 0.9 - splitterWidth) / count - MODE_PADDING);
